                ...
                <h5 class="m-b-10 main-header">{{ $t('jobseeker_list.jobseeker_member_list') }}</h5>               
                    <div class="form-group col-md-12 p-0">                        
                        <div class="input-group">
                            ...
                            />
                            <span class="input-group-addon bg-color" @click="getData();searchClick()">
                        ...
                        <tr v-for="(project, index) in projects.data" :key="project.id">
                            <!-- <td class="text-center">
                                <label class="form-checkbox">
                                    <span v-for="undelete_id in delete_ids_transactions" :key="undelete_id.id" style="color:red;">
                                        {{ undelete_id == project.id ? 'Cannot' : ''}}
                                    </span>
                                    <input type="checkbox" :value="project.id" v-model="selected"/>
                                </label>
                            </td> -->
                            <td class="tbl-wl"><router-link :to="{ path: '/admin/jobseeker-list/jobseeker/'+ project.id +'/detail'}">
                            <span>
                                {{project.jobseeker_number}}
                            </span>
                            </router-link></td>
                            <td class="text-left tbl-xl txt-horizontal-ellipsis"><router-link :to="{ path: '/admin/jobseeker-list/jobseeker/'+ project.id +'/detail'}"><span>{{project.jobseeker_name}} <span v-if="project.login_locked" class="still-lock-label">ロック中</span></span></router-link></td>
                            <td class="tbl-wl">
                                <router-link :to="{ path: '/admin/jobseeker-list/jobseeker/'+ project.id +'/detail'}">
                                    <span>
                                        {{project.email}}
                                    </span>
                                </router-link>
                            ...
                            <td class="tbl-wm">
                                <div class="" v-if="project.record_status != 3 ">
                                    <div class="scout-box">
                                        <span class="btn btn-default" @click="jobseekerEdit(project.id)">{{ $t('common.edit') }}</span>
                                        <!-- <router-link :to="'/admin/jobseeker-list/' + project.id + '/edit'" class="btn btn-default" >{{ $t('common.edit') }}</router-link> -->
                                    </div>
                                </div>
                            </td>
                        </tr>
...
export default {
    ...
    data() {
        let sortOrders = {};
        let columns = this.$i18n.messages.English.jobseeker_list.columns;
    ...
    methods: {
        jobseekerEdit(id){
            ...
            this.paginate.pageName = 'JobseekerEdit'
            this.$store.commit('page',this.paginate)
            this.$router.push({path:'/admin/jobseeker-list/'+id+'/edit'})
            // let getpage = this.projects.to > this.projects.from ? this.projects.current_page : 1;
            // this.$store.commit('page',getpage)
        },
        searchClick() {
