...
export default {
    ...
    methods:{
        ...
        get_taxes(){
            this.$api.get('admin/admin-setting-taxes').then(res => {
                    let taxes = res.data.reduce((x, y) => y);
                    this.current_taxes = taxes
			});
        },
        get_extra_eamil(){
            this.$api.get('admin/get-extra').then(res => {
                    let extra = res.data
                    if(extra == '' || extra == null){
                        this.addEmail = []
                    }else{
        ...
        updateTax(){
            ...
            if(this.current_taxes.percent >= min_length && this.current_taxes.percent != ''){
                this.editTax = false;
                this.$api.post('admin/update-taxes',this.current_taxes).then(() => {
                    this.get_taxes();
                })
            }else{
                this.message = '0以上で入力してください';
        ...
        deleteRow(index) {
            ...
            if(!c_email){
                ...
                this.$api.post(`admin/update-email/${'delete'}`,{oldEmail:c_email}).then( res => {
        ...
        updateEmail(index,currentEmail){
            ...
            if(this.addEmail[index].email != currentEmail){
                this.$api.post(`admin/update-email/${'update'}`,{oldEmail:old_email,c_email:currentEmail}).then( () => {
                    this.addEmail[index].email = currentEmail
                })
            }
             this.addEmail[index].edit = false
        ...
        get_email_status(){
            this.$api.get('admin/email-send').then(res => {
                this.checked = res.data
            })
        },
        changeEmailStatus(e){
            let value = e.target.value
            this.$api.post('admin/update-email-send',{updateEmailSend:value}).then( res => {
                this.checked = res.data
            })
        }   
