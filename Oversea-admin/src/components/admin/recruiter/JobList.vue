                ...
                <div class="content-row m-0">
                    ...
                    <div class="status-row">                        
                        <div class="status-col" v-for="status in arr_status" :key="status.id.id">
                            <div class="mr-3 tooltip-box">
                            <label :for="status.id.display"
                           ...
                           <!-- <span class="tooltiptext">{{status.id.display}}</span>-->
                            <span class="custom-check-label-post"> {{status.id.display}}</span></label>
                            </div>
                        </div>                       
                    </div>
                    <div class="status-row">
                        <div class="status-col tooltip-box">
                            <label for="deactivate" class="custom-control-label custom-checkbox-label">
                              ...
                              <!--<span class="tooltiptext">退会した会員も表示</span>-->
                        </div>
                    </div>
                </div>
                <!--end advanced search-->
        ...
        <div class="row m-0">
            <div class="col-sm-12 p-0">
                ...
                <DataTable
                    ...
                    @sort="sortBy"
                    :showCheckbox="true"
                    class="table-check"  :totalLength="projects.total"
                >
                    <tbody ref="loadingRef">
                        <tr v-for="(project, index) in projects.data" :key="project.id">
                                ...
                                <label class="form-checkbox">
                                 <span v-if="delete_ids_transactions.length > 0" class="tooltip-box" >
                                  <span v-for="undelete_id in delete_ids_transactions" :key="undelete_id.id">
                                    ...
                                    <!-- <span class="tooltiptext">削除できません</span> -->
                            ...
                            <router-link class="d-flex" :to="{ name: 'recruiter-job-detail', params: { id: project.id }}">
                                {{ project.job_number }}
                            ...
                            <td class="tbl-btn">
                                <span class="btn btn-default" @click="jobEdit(project.id)">{{ $t('common.edit')}}</span>
                                <!-- <router-link :to="{ name: 'JobEdit', params: { id: project.id } }" class="btn btn-default">{{ $t('common.edit')}}</router-link> -->
                            </td>
                        </tr>
                    </tbody>
                </DataTable>
...
export default {
    ...
    data() {
        let sortOrders = {};
        let columns = this.$i18n.messages.English.job_list.columns;
    ...
    methods: {
        jobEdit(id){
            ...
            this.paginate.pageName = 'JobEdit'
            this.$store.commit('page',this.paginate)
            this.$router.push({path:'/admin/job-list/'+id+'/edit'})
        },
        searchClick() {
