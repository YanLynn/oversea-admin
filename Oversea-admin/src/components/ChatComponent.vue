<template>
    <div id="boxchat" @mousedown="dragMouseDown">
        <div ref="draggableContainer" :class="`box-chat ${isToggled == true ? 'chat_show' : 'chat_hide' }`">  
                <div class="main-chat col-12">
                    <div class="tab-left float-left">
                        <ul class="list-user">
							<li class="has-input">
                    ...
                    <div class="tab-right float-right">
                        ...
                        <div class="content-chat" ref="scrollChat" v-chat-scroll="{always: false, smooth: true}"  @v-chat-scroll-top-reached="scrollTop(channel,current_page)">
                            <div class="chat-history">
                                ...
                                <ul>
                                    <li v-for="message in messages" :key="message.id" :title="message.created_at | date('%Y-%m-%d %T')">
                                        ...
                                        <div :class="`time ${ isSender(message) ? 'float-right' : ''}`">
                                            {{ message.created_at | xdate(timezone) | date('%Y-%m-%d %T') }}
                                        </div>
...
export default {
	...
	filters: {
		xdate: function(value, tz) {
			...
			let datx = new Date(value).toLocaleString('en-us', { timeZone: tz });
			return datx;
		}
	},
    data(){
    ...
    methods: {
        ...
        markAsRead() {
            this.$api.post(`/messages/read/${this.message_payload.type}/${this.message_payload.scoutid_or_applyid}/${this.currentUser.role_id}`)
            .then(() => {
                this.unreadMessage();
            })
            .catch(err => {
        ...
        sendMessage(e){
			...
			this.$api.post('/messages', this.message_payload)
			.then((r) => {
				this.messages.push({
					recruiter_id: this.message_payload.recruiter_id,
					jobseeker_id: this.message_payload.jobseeker_id,
	...
	computed: {
		...
		timezone() {
			return this.loggedInUser.extra == null || !this.loggedInUser.extra.timezone ? 'Asia/Tokyo' : this.loggedInUser.extra.timezone;
		}
	},
	watch: {
		filter_text() {
